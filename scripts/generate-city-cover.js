/**
 * generate-city-cover.js
 * Generates a branded cover image (PNG) for any city
 * Usage: node scripts/generate-city-cover.js <city-slug>
...
if (!slug) {
  console.error('Usage: node scripts/generate-city-cover.js <city-slug>');
  process.exit(1);
}

// Dynamically import city data
const { getCityBySlug } = await import(
...
if (!city) {
  ...
  const { getAllCities } = await import(
    'file:///' + path.resolve(__dirname, '..', 'src', 'data', 'cityService.js').replace(/\\/g, '/')
  );
...
<div style="width:600px;height:850px;position:relative;overflow:hidden;background:${NAVY2}">
  ...
  <div style="background:#1a2332;padding:40px 48px 32px 52px;min-height:440px;position:relative">
    ...
    <div style="font-size:${cityName.length > 16 ? '34' : '42'}px;font-weight:900;color:${GOLD};letter-spacing:2px;margin:16px 0 6px;line-height:1.1">${cityName}, ${countryName}</div>
  ...
  <div style="background:${NAVY2};padding:40px 52px;min-height:370px;position:relative;overflow:hidden">
    ...
    <div style="position:absolute;bottom:24px;left:52px;right:52px">
      ...
      <div style="display:flex;justify-content:space-between">
        ...
        <span style="font-size:7px;font-weight:600;letter-spacing:1px;color:${GOLD}">${cityName} 2026 -- COST OF LIVING REPORT</span>
      </div>
    </div>
  </div>
</div>
...
async function main() {
  ...
  await pg.setViewport({ width: 600, height: 850 });
  await pg.setContent(coverHTML, { waitUntil: 'domcontentloaded' });

  const outPath = path.resolve(__dirname, '..', 'public', 'images', 'ebooks', `${slug}-cover.png`);
